...
const wrapStyleImports = function (data, isDev, mapping) {
  ...
  if (!isDev) {
    data.links = `${buildLink(mapping['/dist/vendor.css'])}${buildLink(mapping['/dist/'+data.name + '.css'])}`;
  }
};
/**
...
const wrapScriptImports = function (data, isDev, mapping) {
  ...
  if (isDev) {
    ...
    data.scripts += `${buildScript(mapping['/dist/vendor.js'])}`;
    data.scripts += `${buildScript(mapping['/dist/'+data.name+ '.js'])}`;
  }
};
/**
 * 设置通用的响应头,页面不缓存！！
...
const renderTemplateSync = function (request, response, data) {
  ...
  wrapScriptImports(data, isDev, fileMapping);
     ...
     */
  response.end(htmlTemplate(path.join(__dirname, '../views/layout.hbs'))(data));
};
/**
 * 渲染首页
 * 会根据返回数据决定展示分页或系统欢迎页
